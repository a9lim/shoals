...
`congress` (seats + `filibusterActive` + `bigBillStatus` 0–4), `geopolitical` (`chinaRelations` ±3, `gulfEscalation`/`russianCrisis` 0–3, `straitClosed`, `energyCrisis`), `fed`, `investigations` (`tanBowmanStory`/`okaforProbeStage`/`impeachmentStage` 0–3, `meridianExposed` bool — set by bridge event when dirty player is caught in investigative crossfire), `election`, `media` (`tanCredibility`/`sentinelRating` 0–10, `pressFreedomIndex` 0–10, `leakCount` 0–5, `lobbyingExposed` bool — set when `lobbyCount >= 3` and `mediaTrust < 40`).
