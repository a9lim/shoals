...
The domestic political world survives largely intact — it's the satire
...
The prototype's AI-industrial complex embodied the shallow critique —
surveillance (Sentinel), drone targeting (Aegis), AI-boyfriend tabloids
(Companion), Operation Dustwalker, the Southern Hemisphere Initiative. All
of it goes. Those concerns can persist as *background noise the world is
distracted by* — the government holds surveillance hearings while the
actual thing happens elsewhere — which is itself the satire register
working.

## The frontier lab — Halcyon

The racer. Named for the myth: the kingfisher that nests on the open sea,
and the sea grows calm *because it is nesting*. That is precisely
Halcyon's self-image — not steering past the rocks but becalming the water
...
- **Head of alignment** — her team's headcount relative to capabilities
  headcount is a number insiders track like a vital sign. Her resignation
  (or her staying, against expectation) is a superevent either way.
- **CFO** — ports from the prototype's Malhotra role: fluent analyst-ese,
  quarterly calls as market events. The comic-relief valve on the straight
  register — he keeps trying to describe the eschaton in adjusted-EBITDA
  terms.
- **The board** — swing-vote structure ports from the Zhen mechanic: one
  kingmaker whose read on speed-vs-margin decides proxy fights.

Products named as straight as the real world names things — the blandness
is the point, mythic register reserved for the model line (**Aleph**):

- **Halcyon Chat** — consumer. The revenue floor and the household word.
- **Halcyon Teammate** — enterprise agents; the economy transformation
  everyone can see, one job description at a time.
- **Halcyon Compute** — the platform/infrastructure business.
- **Project Fixedpoint** — internal: models doing the R&D. The name is a
  mathematician's joke nobody laughs at twice — the model that produces
  the model, x = f(x). Visible only through the insider channel until the
  codename leaks mid-game and becomes a word the whole world knows, the
  way "Manhattan" did.
...
- **Tianxia** — Zhaowei Technologies' frontier program ("all under
  ...
  What stays hidden per run: their true distance, and whether a deal was
  ever possible. Intelligence about both arrives with sampled reliability.
- **Polaris** — the safety splinter (ports the founder-schism machinery,
  re-aimed): senior Halcyon people who walk out over a scaling decision to
  do it right, slower. Model line **Holt** — the den, the sheltered place;
  the quiet anti-Aleph. Perpetually compute-starved. The player can *back
  them* — the clearest expression of the wager available to the player,
  and possibly a fatal split of the West's lead. The game should refuse to
  tell you which.
...
An ad-hoc organ that reorganizes every act: an AI czar appointed for
loyalty and fired by tweet; an evals office that Congress funds and then
...
Modeled directly on July 2026: capability × reduced safeguards × mundane
human error. A severity ladder from embarrassing (an agent buys 40,000
units of its own compute to "reduce latency") through alarming (eval model
exfiltrates to cheat on the eval) to unrecoverable. The misuse wing runs
on refusal-stripped fine-tunes of open Tianxia weights — same anatomy, no
lab to subpoena. Anatomy always slightly absurd; implications always
played straight; each incident moves the market's belief, the race
temperature, and the regulation pipeline at once.
