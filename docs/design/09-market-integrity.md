...
| Compute futures | precedence: decree conversion (mobilized) ⊃ force-majeure adjustment (blockade) ⊃ ordinary settlement | cash closeout `signedQty × multiplier × (decreePrice − entryBasis)` — shorts owe symmetrically; the committee chooses only among formulas enumerated at listing |
