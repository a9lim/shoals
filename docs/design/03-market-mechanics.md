...
- **HCN — Halcyon common.** Adopted ✓ (2026-07-23): the game's single
  ...
  futures may cover it) — implementation-phase call.
- **Milestone binaries** — prediction-market contracts on **Consensus
  ...
  confirmed." Prices *are* `B` made visible — the market's posterior on
  screen next to yours. Settlement objective; the player's edge is the
  gap. (Also the natural home for Brier-score humility: some of the
  player's dooms won't fire.)
- **Compute futures** — Cambria allocation-quarter contracts. The race's
  physical layer as a term structure: backwardation = scramble.
...
**player's own posterior** — which the player *sets*, explicitly, and
...
- **Act II:** spreads tighten (impact model steepens), events begin
  *pre-pricing* — the market gaps before the headline the player used to
  front-run. The player's informational edge migrates to the insider
  channel and to model quality (their posterior vs. `B`).
- **Act III:** regime break. Correlations converge, VXHCN term structure
  inverts, the underlying drifts away from the GBM+Heston the game's own
  ...
  stutters and repaints, sparklines flicker. Not a shader party — a
...
- Anchor a Cambria capacity expansion (compute nationalism, `C` ↑ broadly)
...
- **Export-control escalation** — slows Tianxia's `C`, heats Serica arcs,
  and now fights a *domestic* constituency: Columbian firms built on free
