/* =====================================================
   ...
   Bodies use plain HTML (no KaTeX -- finsim doesn't load it).
   Unicode symbols for Greek letters and math operators.
   ===================================================== */
...
<p>dS/S = (\u03BC \u2212 q \u2212 \u03BB\u00B7k \u2212 \u00BD\u00B7v)\u00B7dt + \u221Av \u00B7 dW\u2081</p>
<p>The drift term includes the expected return \u03BC, dividend drain q, and a
jump compensator \u03BB\u00B7k that keeps the drift consistent after adding jumps.</p>

<h3>Jump Component</h3>
<p>Jumps arrive as a Poisson process N(\u03BB) with rate \u03BB per year.
Each jump size J is log-normal: ln(1+J) ~ N(\u03BCJ, \u03C3J\u00B2).
Negative \u03BCJ produces a downward skew in the jump distribution.</p>
...
<p>dv = \u03BA(\u03B8 \u2212 v)\u00B7dt + \u03BE\u00B7\u221Av \u00B7 dW\u2082</p>
<p>\u03B8 is the long-run variance level. \u03BA controls the speed of mean reversion.
\u03BE (vol of vol) determines how erratically volatility itself moves.</p>

<h3>Leverage Effect</h3>
<p>The correlation \u03C1 between dW\u2081 and dW\u2082 is typically negative (\u2212 0.5 to \u2212 0.85),
...
<p>When 2\u03BA\u03B8 > \u03BE\u00B2, the variance process stays strictly positive.
...
<h3>Rate Dynamics</h3>
<p>dr = a(b \u2212 r)\u00B7dt + \u03C3R \u00B7 dW\u2083</p>
<p>The parameter a controls mean-reversion speed, b is the long-run equilibrium rate,
and \u03C3R is the rate volatility. dW\u2083 is independent of the stock and variance processes.</p>

<h3>Negative Rates</h3>
<p>Unlike CIR, the Vasicek model permits negative interest rates.
Option pricing uses r_eff = max(r, 10\u207B\u2077) to prevent degenerate boundary behavior
in the Bjerksund-Stensland approximation.</p>

<h3>Bond Pricing</h3>
<p>Zero-coupon bonds are priced as 100 \u00D7 exp(\u2212r \u00D7 T), so bond prices
...
Uses \u03C6 and \u03C8 helper functions with bivariate normal CDF
(Drezner-Wesolowsky 1990, 20-point Gauss-Legendre quadrature).</p>

<h3>Cost of Carry</h3>
<p>b = r \u2212 q, where q is the continuous dividend yield. When b \u2265 r (i.e., q \u2264 0),
early exercise of calls is never optimal, so the model falls back to European Black-Scholes.</p>

<h3>Put-Call Symmetry</h3>
<p>American puts are priced via symmetry: P(S, K, T, r, q, \u03C3) = C(K, S, T, q, r, \u03C3).
...
Deep ITM options approach \u00B11; deep OTM options approach 0.</p>

<h3>Computation</h3>
<p>Central finite difference in S with step h = S \u00D7 0.01.
\u0394 = [V(S+h) \u2212 V(S\u2212h)] / (2h)</p>
...
<h3>Computation</h3>
<p>Second central finite difference in S.
\u0393 = [V(S+h) \u2212 2V(S) + V(S\u2212h)] / h\u00B2</p>
...
<h3>Computation</h3>
<p>Central finite difference in T with step h = 1/252 (one trading day).
\u0398 = [V(T\u2212h) \u2212 V(T+h)] / (2h). Near expiry, the denominator adjusts to prevent
instability.</p>
...
<h3>Computation</h3>
<p>Central finite difference in \u03C3 with step h = 0.001.
\u03BD = [V(\u03C3+h) \u2212 V(\u03C3\u2212h)] / (2h)</p>

<h3>Vega &amp; Heston</h3>
<p>In this simulation, volatility is stochastic (Heston model), so the \u03C3 used for
Greeks is the instantaneous \u221Av, which changes each sub-step. Real vega exposure
...
<h3>Computation</h3>
<p>Central finite difference in r with step h = 0.0001.
\u03C1 = [V(r+h) \u2212 V(r\u2212h)] / (2h)</p>
...
<h3>Spread Model</h3>
<p>halfSpread = max(minHalf, mid \u00D7 pct \u00D7 (1 + \u03C3) + moneyness_weight \u00D7 |ln(S/K)|)</p>
...
equity must be \u2265 50% of debit balance (Reg-T). Trades that would immediately
trigger a margin call are rejected.</p>

<h3>Maintenance Margin</h3>
<p>Short stock/bond: 25% of notional. Short options: max(20% \u00D7 S \u00D7 |qty|, premium \u00D7 |qty|),
...
Status indicator: OK (green) \u2192 LOW (yellow) \u2192 MARGIN CALL (red).</p>

<h3>Borrow Cost</h3>
<p>Short positions and margin debit incur daily interest:
cost = notional \u00D7 (max(r,0) + k\u00B7\u03C3) / 252, where k is the borrow spread parameter.</p>
`,
    },
...
<p>The drift is reduced by q: dS/S includes (\u03BC \u2212 q) instead of \u03BC.
...
dividend per share = S \u00D7 q / 4. Long stock receives cash;
...
<p>Toggle individual Greeks on the legend to overlay \u0394, \u0393, \u0398, \u03BD, \u03C1 curves.
...
<p>Positive \u03BC produces an upward drift in the stock price over time.
Negative \u03BC (as in the Crisis preset) models a bearish environment.
The actual drift in the SDE is \u03BC \u2212 q \u2212 \u03BB\u00B7k \u2212 \u00BD\u00B7v, accounting for
...
`,
    ...
    theta: {
        title: 'Vol Mean (\u03B8)',
        body: `
<p>Long-run variance level in the Heston model. Volatility reverts toward \u221A\u03B8 over time.</p>

<h3>Effect on Volatility</h3>
<p>Higher \u03B8 means a higher baseline volatility environment.
\u03B8 = 0.04 corresponds to ~20% annualised vol; \u03B8 = 0.25 corresponds to ~50% vol.
Combined with \u03BA, determines the persistence of volatility regimes.</p>
...
`,
    ...
    kappa: {
        title: 'Mean Reversion (\u03BA)',
        body: `
<p>Speed at which variance reverts to its long-run mean \u03B8.</p>

<h3>Effect on Dynamics</h3>
<p>High \u03BA pulls volatility back to \u03B8 quickly -- transient vol spikes are short-lived.
Low \u03BA allows volatility to wander far from \u03B8 for extended periods, creating
persistent high-vol or low-vol regimes.</p>

<h3>Feller Condition</h3>
<p>When 2\u03BA\u03B8 > \u03BE\u00B2, variance stays strictly positive. The simulation handles
...
<h3>Effect on Dynamics</h3>
<p>Higher \u03BE creates more erratic volatility swings -- the "vol of vol" effect.
This generates fatter tails in the return distribution and a steeper
volatility smile in option prices.</p>

<h3>Interaction with \u03BA</h3>
<p>Low \u03BA + high \u03BE = persistent wild vol swings (crisis-like).
High \u03BA + low \u03BE = tightly controlled vol near \u03B8 (calm market).</p>
...
<h3>Leverage Effect</h3>
<p>Negative \u03C1 means price drops coincide with vol spikes (and vice versa).
...
<p>More negative \u03C1 produces a steeper volatility skew -- OTM puts become
relatively more expensive. \u03C1 near zero makes the smile more symmetric.</p>
...
<p>Higher \u03BB means more frequent sudden price moves. With \u03BB = 0.5, expect
roughly one jump every two years. With \u03BB = 8 (crisis), expect about
8 jumps per year -- a highly discontinuous price path.</p>

<h3>Jump Compensation</h3>
<p>The drift is adjusted by \u2212\u03BB\u00B7k (where k = E[e^J \u2212 1]) to ensure jumps
...
<p>Negative \u03BCJ (the default) means jumps tend to be downward -- modelling
crash risk. Positive \u03BCJ would model upward jump events (rare in practice
for equities).</p>
...
<p>Higher \u03C3J means jump sizes are more unpredictable -- some jumps might be
...
<p>Higher \u03C3R creates wider rate swings, making bonds more volatile.
...
<h3>Cost Formula</h3>
<p>Daily charge = notional \u00D7 (max(r, 0) + k \u00D7 \u03C3) / 252.</p>
...
<h3>Cash Payments</h3>
<p>Quarterly discrete dividends: S \u00D7 q / 4 per share.
